/* ============================================================
   ...
   2026/07/22 更新:
   - currentExpressionKey を追加。js/idle-motion.js が「待機モーション
     再生前の表情」を保存・復元するために参照する(setExpressionの
...
export class MMDCharacter {
  constructor(mesh, def) {
    ...
    this.exprWeights = {};
    this.exprTargets = {};
    this.currentExpressionKey = 'normal';
  ...
  if (def.type === 'mmd') {
    ...
    loader.load(
      ...
      (mesh) => {
        ...
        // ShadowRig(js/shadow/directional-shadow.js)の光源から影を落とせるようにする。
        // renderer.shadowMap.enabledがfalseの環境(dev.js等、今回は対象外)では無害。
