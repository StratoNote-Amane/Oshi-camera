/* ============================================================
   debug-console.js — 画面内デバッグコンソール(新規)
   ------------------------------------------------------------
   【背景】
   Web Inspector(Mac+ケーブルでのリモートデバッグ)が使えない状況で、
   実機iPhone単体で診断ログを確認・コピーできるようにするための
   最小限のオンスクリーンコンソール。console.log/warn/errorを横取りして
   画面上のパネルにも同時表示する。加えて、投影整合性チェック・環境解析
   状態・距離較正テストをワンタップで実行できるボタンを内蔵し、実機で
   コンソールに何かを「打ち込む」操作自体を一切不要にしている。
...
export function createDebugConsole() {
  ...
  const btnProjection = makeBtn('①投影チェック');
  const btnEnv = makeBtn('環境情報');
  const btnCalib = makeBtn('②③距離較正');
  const btnCopy = makeBtn('コピー');
  const btnClear = makeBtn('クリア');
  const btnClose = makeBtn('閉じる');
  toolbar.append(btnProjection, btnEnv, btnCalib, btnCopy, btnClear, btnClose);
  ...
  // 診断ボタン: main.js側で定義済みのwindow.__verifyProjection() /
  // window.__envAnalyzerState() / window.__runCalibration() を呼ぶだけ。
  // これらは実機のコンソールを一切使わずタップだけで実行できる。
  btnProjection.addEventListener('click', () => {
    if (typeof window.__verifyProjection === 'function') window.__verifyProjection();
    else console.warn('window.__verifyProjection が見つかりません(main.js側の配線を確認してください)');
    show();
  });
  btnEnv.addEventListener('click', () => {
    if (typeof window.__envAnalyzerState === 'function') console.log('[env-analyzer 手動確認]', window.__envAnalyzerState());
    else console.warn('window.__envAnalyzerState が見つかりません');
    show();
  });
  btnCalib.addEventListener('click', () => {
    if (typeof window.__runCalibration === 'function') window.__runCalibration();
    else console.warn('window.__runCalibration が見つかりません');
    show();
  });
