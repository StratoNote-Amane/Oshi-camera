/* ============================================================
   ...
   ------------------------------------------------------------
   2026/08 更新: 「環境光とモデルの色温度が不一致」というフィードバックに
   伴い、lighting.jsのtint反映を強化(estimatedColorTemperatureの反映)
   したのに合わせ、撮影写真全体へ乗せる環境色ティントの強さ(tintAmount)
   も0.12→0.18へわずかに引き上げた。これはlighting.js側のライブ
   プレビュー用の色反映とは別の適用箇所であり(ADR-009/010の通り、
   撮影結果のみに適用される設計は維持)、ここでの調整はキャラクターの
   色そのものではなく「写真全体」への薄い色乗せの強さである点に注意。
   実機未確認の値のため、まだ弱い/強すぎる場合は下記デフォルト値を
   再調整すること。
   ============================================================ */
...
export function applyPhotoFinish(canvas, options = {}) {
  const {
    ...
    contrast = 0.95,
    envTint = null,
    // 2026/08: 0.12→0.18。環境光とモデル色味の不一致フィードバックを受け、
    // 撮影写真全体への環境色ティックをわずかに強めた(実機未確認、たたき台)。
    tintAmount = 0.18,
  } = options;
