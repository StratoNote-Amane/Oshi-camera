...
function applyPixelPass(ctx, w, h, pixels, { chromaticAberration, saturation, contrast, tint }) {
  // 色収差とカラーグレーディングは同じピクセル配列を1回だけ走査して同時に
  // 処理する(静止画1枚とはいえ、2回に分けるより多少軽い)。
  const shift = chromaticAberration ? Math.max(1, Math.round(Math.min(w, h) * 0.0015)) : 0;
