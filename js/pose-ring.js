...
(function () {
  ...
  function init(categoryList, onSelectCallback) {
    ...
    // 2026/07 バグ修正: index.html側はSVGのwidth/height/viewBoxを156(旧R_OUT値)で
    // ハードコードしていたが、R_OUTは88(直径176)に変更済みで、両者が食い違ったまま
    // 放置されていた。syncPosition()はviewBox.baseVal.heightを見て位置を計算するため、
    // このズレがそのまま「リングがシャッター中心からズレて見える」不具合の直接原因
    // だった。R_OUTを唯一の基準にし、ここでSVG側の属性を毎回強制的に上書きすることで、
    // 今後R_OUTを調整してもHTML側の書き換え忘れでズレが再発しないようにする。
    const svg = document.getElementById('ring-svg');
    if (svg) {
      const size = R_OUT * 2;
