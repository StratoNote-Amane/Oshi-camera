...
/* ------------------------------------------------------------
   診断用: クリックした座標で実際に一番上にある要素を特定する。
   キャプチャフェーズ(true)で登録しているので、途中で
   stopPropagation()されていても必ず発火する。
   これでOrbitControlsへドラッグが届かない原因(何が上に
   乗っているか)をCSSを推測せず直接特定できる。
   ------------------------------------------------------------ */
window.addEventListener('pointerdown', (e) => {
  const el = document.elementFromPoint(e.clientX, e.clientY);
  const desc = el ? (el.id ? '#' + el.id : (el.className ? '.' + String(el.className).split(' ').join('.') : el.tagName)) : 'なし';
  logDebug(`[diag] pointerdown座標(${e.clientX},${e.clientY}) の最上位要素: ${desc}`);
}, true);
