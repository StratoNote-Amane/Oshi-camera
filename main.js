...
function buildPoseBar(def) {
  buildPoseBarCore(poseBar, def, () => activeCharacter, (label) => {
    ...
    // ポーズ切り替え時にも接地影の位置を再計算する(実機写真で、ポーズを
    // 変えても影の位置が直立ポーズ時のまま取り残される問題が確認されたため)。
    applyPlacement();
...
stage.addEventListener('touchmove', (e) => {
  ...
  if (touchState.mode === 'drag' && e.touches.length === 1) {
    ...
    // ピンチ(拡縮)・ひねり(回転)と同時に、独立した軸として反映する。
    const rect = stage.getBoundingClientRect();
    const midY = touchMidY(e.touches[0], e.touches[1]);
    const dyMid = midY - touchState.startMidY;
    ...
    placement.z = THREE.MathUtils.clamp(
      touchState.startZ + dyMid * depthPerPixel * DEPTH_DRAG_GAIN,
